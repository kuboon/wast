...
// ---------------------------------------------------------------------------
// Surface — Ruby-flavored WIT type lexemes.
// Plugin-specific lexical choices; everything else lives in wast-syntax-core.
// ---------------------------------------------------------------------------

struct RubyTypePrinter;

impl TypePrinter for RubyTypePrinter {
...
/// Tiny convenience — wraps `wast_syntax_core::format_wit_type` with this
/// plugin's printer. Body code still passes a borrowed `RenderContext`.
fn format_type_ref(type_ref: &WitTypeRef, ctx: &RenderContext) -> String {
    wast_syntax_core::resolve_type_ref(type_ref, ctx, &RubyTypePrinter)
}
...
fn func_to_text(func_uid: &str, func: &WastFunc, ctx: &RenderContext) -> Result<String, String> {
    ...
    let param_types: Vec<String> = func
        .params
        .iter()
        .map(|(_, t)| format_type_ref(t, ctx))
        .collect();
    let param_names: Vec<String> = func
        .params
        .iter()
        .map(|(uid, _)| ctx.local_name(uid).to_string())
        .collect();
    let result_str = match &func.result {
        Some(t) => format!(" -> {}", format_type_ref(t, ctx)),
        None => " -> void".to_string(),
    };
    ...
    match &func.source {
        FuncSource::Imported(_) => {
            // Imports have no body in wast; render as an annotated stub.
...
mod tests {
    ...
    //
    // ruby-like is a renderer-only plugin: there is no from_text, so these
    ...
    #[test]
    fn test_body_renders_string_literal_with_keywords() {
        // Keywords inside string literals must render verbatim.
        assert_body_renders(vec![
            Instruction::LocalSet {
                uid: "v1".into(),
