...
| compiler | `crates/compiler/` | **v0.32 done** (heterogeneous variant construction — payload offset fix) | heterogeneous memory copy → f/i reinterpret → imported resource extras |
...
- [ ] Roadmap: heterogeneous memory copy (emit_copy_from_local disc branch) → f/i reinterpret → imported-resource extras (static methods, resource as field)
