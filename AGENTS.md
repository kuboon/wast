...
| compiler | `crates/compiler/` | **v0.17 done** (+ general `variant` with N cases, VariantCtor/MatchVariant) | tuple → flags/enum → nested compound → resource |
...
- [ ] Roadmap: `tuple` → `flags/enum` → nested compound (record-of-string etc.) → `resource`
