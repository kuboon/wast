...
| compiler | `crates/compiler/` | **v0.25 done** (LocalGet of option/result/variant in field position) | imported resource → heterogeneous join → multi-slot payload copy |
...
- [ ] Roadmap: imported resource → heterogeneous join → multi-slot payload copy
