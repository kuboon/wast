...
| compiler | `crates/compiler/` | **v0.33 done** (variant_like_copy uniform multi-slot) | mixed-width slot copy → f/i reinterpret → imported resource extras |
...
- [ ] Roadmap: mixed-width joined-slot copy (disc branch + per-case writes) → f/i reinterpret → imported-resource extras
