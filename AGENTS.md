...
| compiler | `crates/compiler/` | **v0.31 done** (implicit widen at LocalGet) | construct-side widen → heterogeneous memory copy → f/i reinterpret |
...
- [ ] Roadmap: construct-side widen (Some/Ok/Err/VariantCtor narrow payload in wider join) → heterogeneous memory copy → f/i reinterpret
